...
import { InterruptEvent } from "@/lib/scenarios";
...
export function InterruptOverlay({ event, onChoose }: Props) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setRemaining(r => {
        const n = Math.max(0, r - 0.1);
        if (n === 0) onChoose(event.options[0].id);
        return n;
      });
    }, 100);
    ...
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex items-center justify-center px-6 animate-flash">
      <div className="max-w-xl w-full">
        ...
        <div className="mt-6 h-0.5 bg-secondary overflow-hidden">
          <div className="h-full bg-destructive transition-all" style={{ width: `${(remaining / event.timeLimit) * 100}%` }} />
        </div>
      </div>
    </div>
  );
